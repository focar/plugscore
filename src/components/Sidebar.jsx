...
// Ícone para o botão de logout (sem alterações)
...
export default function Sidebar({ userProfile }) {
    ...
    const { isMobileMenuOpen, setIsMobileMenuOpen } = useContext(AppContext); // Pegar estado do menu

    // ... (lógica de menu e logout, sem alterações) ...
    const allMenuItems = {
         DASHBOARDS: [
     ...
     const handleLinkClick = () => {
        setIsMobileMenuOpen(false); // Fecha o menu ao clicar em um link
     };
    ...
    return (
        <>
            ...
            {/* A Sidebar em si */}
            <div className={`fixed top-0 left-0 h-screen w-64 bg-gray-900 text-gray-300 flex flex-col p-4 z-40
                            ...
                            <ul>
                                {items.map((item) => (
                                    <li key={item.name}>
                                        <Link
                                            href={item.href}
                                            onClick={handleLinkClick} // Adicionado aqui

...
// Imports e configs são os mesmos da versão de "Mov. Diário" do tráfego pago...
...
const DailyMovementChart = ({ data }) => {
  if (data.length === 0) { return (<div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg flex flex-col items-center justify-center h-[40rem]"><p className="text-gray-500 dark:text-slate-400">Nenhum dado de movimento diário para exibir.</p></div>); }
  const formattedData = data.map(item => ({...item, dateFormatted: new Date(item.movement_date).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', timeZone: 'UTC' }), }));
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg h-[40rem]">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={formattedData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <Tooltip contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', border: '1px solid #ccc', backdropFilter: 'blur(5px)' }} wrapperClassName="dark"/>
          <Legend />
          <Bar dataKey="leads_count" name="Leads" fill="#22c55e" />
          <Bar dataKey="checkins_count" name="Check-ins" fill="#16a34a" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
...
function MovDiarioPageContent() {
  ...
  const fetchData = useCallback(async (id) => {
    ...
    try {
      ...
      // --- ALTERAÇÃO: Chama a função SQL de Mov. Diário ORGÂNICO ---

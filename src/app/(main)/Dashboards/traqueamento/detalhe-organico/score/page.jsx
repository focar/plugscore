...
// Imports e configs são os mesmos da versão de "score" do tráfego pago...
...
function ScorePageContent() {
  ...
  const fetchData = useCallback(async (id) => {
    ...
    try {
      ...
      // --- ALTERAÇÃO: Chama a função SQL de score ORGÂNICO ---
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      ) : !data || !data.totals ? ( <div className="text-center py-16 text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 rounded-lg shadow-md">Nenhum dado de score para este lançamento.</div>
      ) : (
        <main>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
             {Object.keys(SCORE_CONFIG).map(key => (<ScoreBreakdownChart key={key} title={`${SCORE_CONFIG[key].title} ${SCORE_CONFIG[key].range}`} data={summarizeChartData(data?.breakdowns?.[key])} />))}
          </section>
        </main>
      )}
    </div>

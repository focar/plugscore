...
// Usando o caminho original que usa o alias '@'
import { AppContext } from '@/context/AppContext'; 
import { TrendingUp, TrendingDown, Eye, CheckCircle, XCircle, AlertCircle, Hash, Percent, Target, UserCheck } from 'lucide-react';
...
// Componente KpiCard: Nenhuma alteração necessária, já é responsivo.
...
// Componente RecommendationBadge: Nenhuma alteração necessária.
...
// Componente ScoreDistributionDisplay: Nenhuma alteração necessária, 'flex-wrap' cuida da responsividade.
...
// --- Componente de Item para a Lista Mobile ---
// Este é um novo componente (criado aqui mesmo) para renderizar cada criativo
// como um "cartão" na visualização mobile.
const MobileCreativeCard = ({ creative }) => (
    <div className="bg-white dark:bg-gray-700/50 rounded-lg shadow-md p-4 space-y-4 border border-gray-200 dark:border-gray-700" title={`Justificativa: ${creative.justificativa}`}>
        {/* Cabeçalho do Cartão */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <h4 className="text-base font-semibold text-gray-900 dark:text-white break-words w-full sm:w-auto">{creative.criativo}</h4>
            <div className="flex-shrink-0">
                <RecommendationBadge recommendation={creative.recomendacao} />
            </div>
        </div>

        {/* Corpo do Cartão com Métricas */}
        <div className="border-t border-gray-200 dark:border-gray-600 pt-4">
            <h5 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Qualidade (Leads)</h5>
            <div className="flex items-center justify-start gap-4 bg-gray-50 dark:bg-gray-800 p-3 rounded-md">
                <div className="text-center">
                    <span className="text-xs text-gray-500 dark:text-gray-400 block">Quentes</span>
                    <span className="text-xl font-bold text-red-600 dark:text-red-400">{creative.leads_quentes}</span>
                </div>
        ...
        <div className="grid grid-cols-3 gap-4 text-center border-t border-gray-200 dark:border-gray-600 pt-4">
            <div>
                ...
                <dd className="text-base font-semibold text-gray-900 dark:text-gray-100 mt-1">{creative.total_inscricoes}</dd>
            </div>
            <div>
                ...
                <dd className="text-base font-semibold text-gray-900 dark:text-gray-100 mt-1">{creative.total_checkins}</dd>
            </div>
            <div>
                ...
                <dd className="text-base font-semibold text-gray-900 dark:text-gray-100 mt-1">{creative.taxa_checkin}%</dd>
            </div>
        </div>
    </div>
);
...
export default function RecomendacaoCriativosPage() {
    ...
    // Lógica para buscar lançamentos (sem alterações)
    useEffect(() => {
    ...
    // Lógica para buscar os dados de recomendação (sem alterações)
    ...
    // Lógica para atualizar o header (sem alterações)
    useEffect(() => {
        const launchSelector = (
            // MUDANÇA: Removido 'max-w-xs'. O Header agora controla a largura.
    ...
    // Lógica de contagem (sem alterações)
    ...
    return (
        // Padding responsivo: p-4 em telas pequenas, sm:p-6 em maiores
        <div className="p-4 sm:p-6 lg:p-8">
            {/* Seção de Resumo: Nenhuma alteração necessária, 'grid-cols-1 md:grid-cols-3' já é responsivo */}
            ...
            {/* --- MUDANÇA PRINCIPAL: ÁREA DA TABELA --- */}
            <main className="bg-white dark:bg-gray-800 p-0 sm:p-0 rounded-lg shadow-lg">
                {loading ? <Spinner /> : (
                    <>
                        {data.length === 0 ? (
                            <div className="text-center p-8 text-gray-500">Nenhum criativo de tráfego pago encontrado para este lançamento.</div>
                        ) : (
                            <>
                                {/* --- 1. VISUALIZAÇÃO EM TABELA (Desktop) --- */}
                                {/* 'hidden' esconde em telas pequenas, 'md:block' exibe em telas médias e maiores */}
                                ...
                                {/* 'block' exibe em telas pequenas, 'md:hidden' esconde em telas médias e maiores */}
                                {/* Adicionei 'p-4' aqui para dar um respiro no mobile, já que o 'main' agora tem p-0 */}

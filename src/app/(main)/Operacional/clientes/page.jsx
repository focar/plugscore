...
// --- Componentes (Input, Modal sem alterações) ---
...
export default function ClientesPage() {
    ...
    // ... (fetchData, handleAddNew, handleEdit, handleDelete, handleSave sem alterações)
    async function fetchData() { setLoading(true); setError(null); try { const { data, error } = await supabase.from('clientes').select('*, lancamentos(codigo, nome)').order('nome', { ascending: true }); if (error) throw error; setClientes(data || []); } catch (err) { setError('Erro ao carregar clientes: ' + err.message); } finally { setLoading(false); } }
    const handleAddNew = () => { setEditingCliente({ nome: '', codigo: '', lancamentos: [] }); setIsModalOpen(true); };
    const handleEdit = (cliente) => { setEditingCliente({ ...cliente }); setIsModalOpen(true); };
    const handleDelete = async (clienteId) => { if (!window.confirm("Tem a certeza que deseja apagar este cliente? Todos os lançamentos e dados associados podem ser afetados.")) return; try { await supabase.from('clientes').delete().eq('id', clienteId).throwOnError(); fetchData(); } catch (err) { setError("Não foi possível apagar o cliente. Verifique se existem lançamentos associados a ele."); } };
    const handleSave = async (e) => { e.preventDefault(); if (isSaving) return; setIsSaving(true); setError(null); const { lancamentos, ...dataToSave } = editingCliente; dataToSave.codigo = dataToSave.codigo.toUpperCase(); try { if (dataToSave.id) { await supabase.from('clientes').update({ nome: dataToSave.nome, codigo: dataToSave.codigo }).eq('id', dataToSave.id).throwOnError(); } else { await supabase.from('clientes').insert({ nome: dataToSave.nome, codigo: dataToSave.codigo }).throwOnError(); } setIsModalOpen(false); fetchData(); } catch (err) { setError("Não foi possível guardar o cliente: " + err.message); } finally { setIsSaving(false); } };
        ...
        <div className="p-4 md:p-8 overflow-hidden">
            <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg">
                ...
                <div>
                    ...
                    clientes.length === 0 ? <p className="text-center py-8 text-gray-500 dark:text-gray-400">Nenhum cliente encontrado.</p> :
                    (
                        <>
                            {/* Tabela Desktop */}
                                ...
                                {clientes.map((cliente) => (
                                    <div key={cliente.id} className="p-4">
                                        <div className="flex justify-between items-center mb-1">
                                            <span className="text-lg font-semibold text-gray-900 dark:text-white">{cliente.nome}</span>
                                        </div>
                                         <div className="text-sm mb-3">
                                            <span className="font-mono text-gray-500 dark:text-gray-400 mr-2 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">
                                                Cod: {cliente.codigo}
                                            </span>
                                             {cliente.lancamentos.length > 0 && (
                                                <span className="text-gray-500 dark:text-gray-400 italic">
                                                    Lanç: {cliente.lancamentos.map(l => l.codigo).join(', ')}
                                                </span>
                                             )}
                                        </div>
